...
1. Transforme TOUS les tableaux au format JSON suivant ces règles:
   - Réorganise les données pour avoir une colonne principale #Produit qui servira d'identifiant
   - Convertis les colonnes redondantes (comme les tensions 120V, 240V) en variables attributs
   - Empile verticalement les produits similaires qui ont des variations mineures
   - Présente chaque produit comme un objet JSON avec toutes ses caractéristiques
   - Regroupe tous les produits dans un tableau d'objets
...
"""
        ...
        with client.messages.stream(
            ...
            tous les tableaux en format JSON structuré selon ces principes:
            1. Réorganise les données pour avoir #Produit comme identifiant primaire
            2. Convertis les colonnes redondantes (comme tensions différentes) en variables attributs
            3. Regroupe les produits similaires ayant des variations mineures
            4. Structure chaque produit comme un objet JSON avec toutes ses caractéristiques
            5. Présente une collection de produits dans un tableau d'objets JSON bien formaté
            
            Entre chaque tableau et section, ajoute des séparateurs "-------" pour faciliter l'organisation.
            Pour chaque tableau, indique clairement le code et nom du produit au-dessus.""",
            messages=[
